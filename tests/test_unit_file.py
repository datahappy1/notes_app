...
<section=second> Quis istum dolorem timet"""
    ...
    def test_save_file_data(self, get_file):
        get_file._raw_data_content = """<section=first> Quod equidem non reprehendo
<section=second> Quis istum dolorem timet"""
        assert (
            get_file.save_file_data() is None
        )

    def test_save_file_data_handle_error(self, get_app, get_file):
        # test case to cover scenario when data write fails no data is lost
        get_file._raw_data_content = """<section=first> Quod equidem non reprehendo
        <section=second> Quis istum dolorem timet <section=third> !"""
        get_file._data_by_sections = get_file._transform_raw_data_content_to_data_by_sections()

        get_file._file_path = None
        # writing to file through save_file_data raises Exception
        with pytest.raises(Exception):
            get_file.save_file_data()
